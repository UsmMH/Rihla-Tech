...
- **Home dashboard** — plan a new trip, ask AI for travel advice, resume recent trips
- **My Trips** — list, reopen, and delete past itineraries
...
- **App shell** — Home · My Trips · Community (coming soon) · Profile with theme toggle
...
Trip result → Ask AI (trip chat) → propose edit → Apply / "yes" → itinerary updates
            → Open in Maps / Day route
```
...
| **Result** | Day-by-day AI itinerary; per-activity and per-day Maps links |
...
flowchart TB
    ...
    subgraph Server["FastAPI backend"]
        ...
        SERVICES[Services: LLM · itinerary · edit · chat]
        ROUTERS --> SERVICES
    end
...
| 6 — Flights/hotels | — | Flight search + hotel cards with booking links |
...
# 1. Environment
cp .env.example .env
# Fill in required keys (JWT, LLM, etc.)
...
src/
  ...
  components/trip/     ChatbotSidebar, QuestionFlow
  components/layout/   Navbar, AppBottomNav, Footer
  components/auth/     Login / register
  lib/                 API client, auth, trips, places, mapDirections
backend/
  app/
    routers/           auth, quiz, trips, places, chat, health
    services/          llm, itinerary, destinations, edit, apply_edit, chat, consult_chat
...
- **UI polish** — further responsive and visual refinements planned after Phase 6
