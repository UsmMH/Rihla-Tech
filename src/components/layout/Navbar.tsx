...
const APP_NAV_ITEMS: { label: string; tab: AppTab }[] = [
  { label: "Home", tab: "home" },
...
export default function Navbar({
  ...
  function profileButton(className: string) {
    ...
    return (
      <button
        ...
        className={className}
        style={{
          background: isActive ? theme.badgeBg : theme.toggleBg,
          border: "none",
        }}
        aria-label="Profile"
        aria-current={isActive ? "page" : undefined}
      >
        <User size={18} color={isActive ? theme.accentSky : theme.toggleIcon} strokeWidth={2} />
      </button>
    );
  }
  ...
  return (
    <>
      ...
      >
        <div className="max-w-5xl mx-auto w-full flex items-center gap-4 px-4 md:px-6 py-3.5 min-h-[60px]">
              ...
              {APP_NAV_ITEMS.map((item) => {
                ...
                return (
                  <button
                    ...
                    className="text-sm px-3 py-2 rounded-lg transition-colors cursor-pointer whitespace-nowrap"
                    style={{
            ...
            {isMarketing ? (
              ...
              <>
                <button
                  type="button"
                  onClick={() => goToTab("profile")}
                  className="hidden md:inline-flex text-sm px-3 py-2 rounded-lg transition-colors cursor-pointer whitespace-nowrap"
                  style={{
                    color: activePage === "profile" ? theme.accentSky : theme.navLinkText,
                    background: activePage === "profile" ? theme.badgeBg : "none",
                    border: "none",
                    fontFamily: "system-ui, sans-serif",
                ...
                {profileButton("w-9 h-9 flex items-center justify-center rounded-lg cursor-pointer md:hidden")}
              </>
            )}
          </div>
        </div>
